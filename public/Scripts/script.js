...
function createElementHandler(data) {
    ...
    data.forecast.forEach(function (item) {
        nextDaysContainer.insertAdjacentHTML('beforeend',
            `
        <div class="max-sm:gap-4 max-md:flex-row flex animate-fadeIn flex-col  items-center justify-between">
            <p class="max-md:text-center max-sm:text-[12px] max-md:text-[12px] max-lg:text-[14.5px] max-xl:text-[16px] text-white font-black text-xl">${item.day}</p>
            <img class="max-sm:size-12 max-md:size-20 max-lg:size-30 size-44" src="${item.icon}" alt="">
            <div class="flex flex-col items-center">
                <p class="max-sm:text-[12px] max-md:text-[12px] max-lg:text-[14.5px] max-xl:text-[16px] text-white font-bold">${item.temp}°C - ${Math.floor(calculateFarenheit(item.temp))}°F</p>
                <p class="max-sm:text-[12px] max-md:text-[12px] max-lg:text-[14.5px] max-xl:text-[16px] text-white font-black">${item.condition}</p>
            </div>
        </div>
        `
        )
